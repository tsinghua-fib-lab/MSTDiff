    ...
    def __init__(self, in_channels, num_hiddens, num_residual_hiddens):
        ...
        self._block = nn.Sequential(
            ...
            nn.Conv1d(in_channels=in_channels,
                      ...
                      kernel_size=3, stride=1, padding=1, bias=False),  # L -> L
            nn.ReLU(True),
            nn.Conv1d(in_channels=num_residual_hiddens,
                      out_channels=num_hiddens,
                      kernel_size=1, stride=1, bias=False)  # L -> L
        )
...
class ResidualStack(nn.Module):
    def __init__(self, in_channels, num_hiddens, num_residual_layers, num_residual_hiddens):  # 这里设置num_residual_layers=2
...
class Encoder(nn.Module):
    def __init__(self, in_channels, num_hiddens, num_residual_layers, num_residual_hiddens, embedding_dim, compression_factor):
        ...
        if compression_factor == 4:
            # in_channels到num_hidden是升维
            self._conv_1 = nn.Conv1d(in_channels=in_channels,
                                     out_channels=num_hiddens // 2,
                                     kernel_size=4,
                                     stride=2, padding=1)  # L -> L/2
            self._conv_2 = nn.Conv1d(in_channels=num_hiddens // 2,
                                     out_channels=num_hiddens,
                                     kernel_size=4,
                                     stride=2, padding=1)  # L/2 -> L/4
            self._conv_3 = nn.Conv1d(in_channels=num_hiddens,
                                     out_channels=num_hiddens,
                                     kernel_size=3,
                                     stride=1, padding=1)  # L/4 ->  L/4
            ...
            self._conv_A = nn.Conv1d(in_channels=num_hiddens,
                                     out_channels=num_hiddens,
                                     kernel_size=4,
                                     stride=2, padding=1)   # L/4 -> L/8
            self._conv_3 = nn.Conv1d(in_channels=num_hiddens,
                                     out_channels=num_hiddens,
                                     kernel_size=3,
                                     stride=1, padding=1)   # L/8 -> L/8
            self._residual_stack = ResidualStack(in_channels=num_hiddens,
                                                 num_hiddens=num_hiddens,
                                                 num_residual_layers=num_residual_layers,
                                                 num_residual_hiddens=num_residual_hiddens)
...
class VectorQuantizer(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, commitment_cost):
        ...
        self._embedding_dim = embedding_dim  # 32
        self._num_embeddings = num_embeddings  # 128
...
class vqvae(BaseModel):
    ...
    def shared_eval(self, batch, optimizer, mode):
        if mode == 'train':
            ...
            # print("shape ", data_recon.shape, batch.shape)
